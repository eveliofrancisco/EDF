...
c-----------------------------------------------------------------------
c
      subroutine casoqsloc (aominp,mxcent,covx,epsneg,maxb,nloc,
     & nloctarget,udat,wfnfile,maxcritov,critov,ixcent,largwr,warn,
     & skiph,lw,lerr)
c
c-----PARAMETERS--------------------------------------------------------
c
...
c-----------------------------------------------------------------------
c
      call timer (2,iopenloc,'_casoqsloc',-1)
...
c
            ...
            call errdsyev (info,'casoqsloc.f')
            uvec = sg
c
...
c
      ...
      ocup(1:nloc)=2d0
      if (ier.ne.0) stop '# casoqsloc.f: Cannot allocate ocup()'
...
c
      ...
      if (ier.ne.0) stop '# casoqsloc.f: Cannot deallocate ocup()'
c
...
c
      return
c
      call timer (4,iopenloc,'_casoqsloc',-1)
c
c.....Formats
c
 410  format (' # ',I4)
 ...
 434  format (' #',/,' # casoqsloc.f: Unable to localize all the MOs',
     & /,' #',/,' # NON CONVERGED (1--N)-CENTER OPEN SYSTEM ANALYSIS',
     & /,' #')
 435  format (' #',
     & /,' # casoqsloc.f: ! WARNING: Too many localized MOs !',/,' #')
 100  format (' # casoqsloc.f: File ',a,' NOT FOUND')
