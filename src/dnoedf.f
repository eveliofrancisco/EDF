...
c-----------------------------------------------------------------------
c
      subroutine dnoedf (nmo,ncent,naom,ngroup,nfugrp,ifugrp,pcut,
     &                   epsbond,lr,lw,lerr,lu18)
c
c.......................................................................
c
...
c
      ...
      if (.not.allocated(aom)) then
        ...
        if (ier.ne.0) stop 'dnoedf.f: Cannot allocate aom()'
      endif
...
c
      if (.not.allocated(sg)) then
        ...
        if (ier.ne.0) stop 'dnoedf.f: Cannot allocate sg()'
      endif
...
c
      if (.not.allocated(sdiag)) then
        ...
        if (ier.ne.0) stop 'dnoedf.f: Cannot allocate sdiag()'
      endif
      if (.not.allocated(bdiag)) then
        allocate (bdiag(nmo,nmo),stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot allocate bdiag()'
      endif
      if (.not.allocated(seigen)) then
        allocate (seigen(nmo),stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot allocate seigen()'
      endif
      if (.not.allocated(wdiag)) then
        allocate (wdiag(nmo+nmo-1),stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot allocate wdiag()'
      endif
c
      do i=1,nmo
        do j=1,i
          val1=cmplx(zero,zero)
          val2=cmplx(zero,zero)
          do k=1,nmo
            val1=val1+conjg(sg(1,k,i))*sg(2,k,j)
            val2=val2+conjg(sg(2,k,i))*sg(1,k,j)
          enddo
          sdiag(i,j)=val1+val2
...
c
      if (.not.allocated(eignz)) then
        ...
        if (ier.ne.0) stop 'dnoedf.f: Cannot allocate eignz()'
      endif
      nzeig=izero
      do i=1,nmo
...
c
      if (.not.allocated(sgn)) then
        ...
        if (ier.ne.0) stop 'dnoedf.f: Cannot allocate sgn()'
      endif
      do k=1,ngroup-1
        do i=1,nzeig
          i1=eignz(i)
...
c
      if (.not.allocated(resncb)) then
        ...
        if (ier.ne.0) stop 'dnoedf.f: Cannot allocate resncb()'
      endif
c
...
c
      ...
      if (nprev.lt.npab) then
        write (stderr,*) 'dnoedf.f: NPREV < NPAB returned by rnprobs.f'
        stop
      endif
c
      if (.not.allocated(resnca)) then
        allocate (resnca(npab,ngroup),stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot allocate resnca()'
      endif
...
c
      if (allocated(aom)) then
        ...
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate aom()'
      endif
      if (allocated(sg)) then
        deallocate (sg,stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate sg()'
      endif
      if (allocated(sdiag)) then
        deallocate (sdiag,stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate sdiag()'
      endif
      if (allocated(bdiag)) then
        deallocate (bdiag,stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate bdiag()'
      endif
      if (allocated(seigen)) then
        deallocate (seigen,stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate seigen()'
      endif
      if (allocated(wdiag)) then
        deallocate (wdiag,stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate wdiag()'
      endif
      if (allocated(eignz)) then
        deallocate (eignz,stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate eignz()'
      endif
      if (allocated(sgn)) then
        deallocate (sgn,stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate sgn()'
      endif
      if (allocated(eleca)) then
        deallocate (eleca,stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate eleca()'
      endif
      if (allocated(resncb)) then
        deallocate (resncb,stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate resncb()'
      endif
      if (allocated(resnca)) then
        deallocate (resnca,stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate resnca()'
      endif
      if (allocated(mocore)) then
        deallocate (mocore,stat=ier)
        if (ier.ne.0) stop 'dnoedf.f: Cannot deallocate mocore()'
      endif
c
      call timer (4,idno,'_dnoedf   ',-1)
      return
c
 ...
 112  format (/,' # ',70('+'),/,' #',25x,'DNOEDF MODULE',/,
     & ' # ',70('+'),/,' # Eigenvalues of (SA SB + SB SA) matrix')
      end
