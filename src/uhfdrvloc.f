...
c-----------------------------------------------------------------------
c
      subroutine uhfdrvloc (aominp,mxcent,covx,epsneg,maxb,mal,mbe,
     &  nloctot,udat,wfnfile,maxcritov,critov,ixcent,largwr,warn,
     &  skiph,lw,lerr)
c
c-----------------------------------------------------------------------
c                         
...
c-----MAL,MBE                        INPUT
c
c     Total number of localized ALPHA and BETA electrons that should be 
c     found 
c
c-----NLOCTOT                        OUTPUT
c
c     Total number of Localized MOs (alpha+beta) found. It should be
c     equal to MAL+MBE at the end of the process.
c
c.....UDAT                           INPUT
c
...
c                         
      ...
      real   (kind=8),allocatable, dimension (:,:)   :: sumaom,ornor
...
C           if (ios.ne.0) exit
C           if (m.gt.nmo.or.n.gt.nmo) then
C             stop '# uhfdrvloc.f: Bad indices of rho() matrix'
C           endif
C           rho(m,n) = value
C           rho(n,m) = value
C         enddo
C       endif
        ...
        sumaom=0d0
        do k=1,ncent
           sumaom(:,:)=sumaom(:,:)+aomloc(k,:,:)
        enddo
        call uhfoqsloc (aomloc,aomloco,rhosig,c,co,code,
     &    nab,nloctarget,mxcent,covx,epsneg,maxb,nloc,udat,wfnfile,
     &    maxcritov,critov,ixcent,largwr,warn,skiph,lw,lerr)

        if (nloc.ne.nab) then
          stop ' uhfdrvloc.f: Failed localization of Canonical MOs'
        endif
c
c-----------------------------------------------------------------------
c
...
c
        inic=naddmo+1
        nfin=naddmo+nloc
        if (code.eq.1) ca=c(1:nloc,1:nab)
        if (code.eq.2) cb=c(1:nloc,1:nab)
        if (code.eq.1) cao=co(1:nloc,1:nab)
        if (code.eq.2) cbo=co(1:nloc,1:nab)
        do i=1,nloc
          do j=1,nprims
            value1=0d0
            value2=0d0
            ...
            enddo
            cc(i+naddmo,j)  = value1
            cco(i+naddmo,j) = value2
          enddo
        enddo
c
...
c
        ...
        ornor=matmul(c(1:nloc,1:nab),transpose(co(1:nloc,1:nab)))
        ...
        deallocate (rhosig, aomloc, aomloco, sumaom, ornor, c, co)
      enddo
c
...
c
      ...
      if (ier.ne.0) stop '# uhfdrvloc.f: Cannot allocate ocup()'
...
c
      ...
      if (ier.ne.0) stop '# uhfdrvloc.f: Cannot deallocate ocup()'
c
...
c
 ...
 100  format (' # uhfdrvloc.f: File ',a,' NOT FOUND')
