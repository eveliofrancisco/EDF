...
c--------------------------------------------------------------------
c
c-----MP is the logical output unit.
c
c-----LP is the logical unit for the printing of error messages.
c     (no error messages are written if LP < 0).
c
...
C
      if (IPRINT(1).GE.0) then
        CALL LB1(IPRINT,ITER,NFUN,GNORM,N,M,X,F,G,STP,FINISH,MP)
      endif
C
C    --------------------
...
C     ----------------------------------------------------
 ...
 172  CONTINUE
     ...
     &  XTOL,MAXFEV,INFO,NFEV,DIAG,gtol,stpmin,stpmax,LP)
      IF (INFO .EQ. -1) THEN
        IFLAG=1
        RETURN
      ENDIF
...
C
 190  IFLAG=-1
ccc   if (LP.GT.0) write (LP,200) INFO
      RETURN
 195  IFLAG=-2
...
c-----------------------------------------------------------------------
c
      SUBROUTINE LB1(IPRINT,ITER,NFUN,GNORM,N,M,X,F,G,STP,FINISH,MP)
C
C     -------------------------------------------------------------
...
C
 10   FORMAT(1x,80('+'))
 20   FORMAT(1x,'N =',I5,'   NUMBER OF CORRECTIONS =',I2)
 30   FORMAT(1x,'INITIAL VALUES:',/,
     &   1x,'Function = ',1PD10.3,4x,'Gradient NORM = ',1PD10.3)
 40   FORMAT(' VECTOR X = ')
 ...
 100  FORMAT(/' LBFGS --> MINIMIZATION ENDS WITHOUT DETECTING ERRORS.')
C
      RETURN
      END
C     ******
