...
c*********************************************************************72
c
cc GEN_OH generates points under OH symmetry.
c
c  Discussion:
c
c    Given a point on a sphere, specified by A and B, this routine generates
c    all the equivalent points under OH symmetry, making grid points with
c    weight V.
c
c    The variable NUM is increased by the number of different points
c    generated.
c
c    Depending on CODE, there are from 6 to 48 different but equivalent
c    points that are generated:
c
c      CODE=1:   (0,0,1) etc                                (  6 points)
c      CODE=2:   (0,A,A) etc, A=1/sqrt(2)                   ( 12 points)
c      CODE=3:   (A,A,A) etc, A=1/sqrt(3)                   (  8 points)
c      CODE=4:   (A,A,B) etc, B=sqrt(1-2 A^2)               ( 24 points)
c      CODE=5:   (A,B,0) etc, B=sqrt(1-A^2), A input        ( 24 points)
c      CODE=6:   (A,B,C) etc, C=sqrt(1-A^2-B^2), A, B input ( 48 points)
c
c  Modified:
c
c    11 September 2010
...
c  Parameters:
c
c    Input, integer CODE, selects the symmetry group.
c
c    Input/output, integer NUM, presumably a counter for the total number
c    of points.  It is incremented by the number of points generated on
c    this call.
c
c    Input, double precision A, B, information that may be needed to
c    generate the coordinates of the points (for code = 5 or 6 only).
c
c    Input, double precision V, the weight to be assigned the points.
c
c    Output, double precision X(NUM), Y(NUM), Z(NUM), W(NUM), the coordinates
c    and weights of the symmetric points generated on this call.
c
      implicit none
...
c*********************************************************************72
c
cc LD_BY_ORDER returns a Lebedev angular grid given its order.
c
c  Discussion:
c
c    Only a certain set of such rules are available through this function.
c
c  Modified:
c
c    13 September 2010
...
c
c    Output, double precision X(ORDER), Y(ORDER), Z(ORDER), W(ORDER), 
c    the coordinates and weights of the points.
c
      implicit none
      ...
      if ( order .eq. 6 ) then
        ...
        call ld0014 ( x, y, z, w )
      else if ( order .eq. 26 ) then
        call ld0026 ( x, y, z, w )
      else if ( order .eq. 38 ) then
        call ld0038 ( x, y, z, w )
      else if ( order .eq. 50 ) then
        call ld0050 ( x, y, z, w )
      else if ( order .eq. 74 ) then
        call ld0074 ( x, y, z, w )
      else if ( order .eq. 86 ) then
        call ld0086 ( x, y, z, w )
      else if ( order .eq. 110 ) then
        call ld0110 ( x, y, z, w )
      else if ( order .eq. 146 ) then
        call ld0146 ( x, y, z, w )
      else if ( order .eq. 170 ) then
        call ld0170 ( x, y, z, w )
      else if ( order .eq. 194 ) then
        call ld0194 ( x, y, z, w )
      else if ( order .eq. 230 ) then
        call ld0230 ( x, y, z, w )
      else if ( order .eq. 266 ) then
        call ld0266 ( x, y, z, w )
      else if ( order .eq. 302 ) then
        call ld0302 ( x, y, z, w )
      else if ( order .eq. 350 ) then
        call ld0350 ( x, y, z, w )
      else if ( order .eq. 434 ) then
        call ld0434 ( x, y, z, w )
      else if ( order .eq. 590 ) then
        call ld0590 ( x, y, z, w )
      else if ( order .eq. 770 ) then
        call ld0770 ( x, y, z, w )
      else if ( order .eq. 974 ) then
         call ld0974 ( x, y, z, w )
      else if ( order .eq. 1202 ) then
        call ld1202 ( x, y, z, w )
      else if ( order .eq. 1454 ) then
        call ld1454 ( x, y, z, w )
      else if ( order .eq. 1730 ) then
        call ld1730 ( x, y, z, w )
      else if ( order .eq. 2030 ) then
        call ld2030 ( x, y, z, w )
      else if ( order .eq. 2354 ) then
        call ld2354 ( x, y, z, w )
      else if ( order .eq. 2702 ) then
        call ld2702 ( x, y, z, w )
      else if ( order .eq. 3074 ) then
        call ld3074 ( x, y, z, w )
      else if ( order .eq. 3470 ) then
        call ld3470 ( x, y, z, w )
      else if ( order .eq. 3890 ) then
        call ld3890 ( x, y, z, w )
      else if ( order .eq. 4334 ) then
        call ld4334 ( x, y, z, w )
      else if ( order .eq. 4802 ) then
        call ld4802 ( x, y, z, w )
      else if ( order .eq. 5294 ) then
        call ld5294 ( x, y, z, w )
      else if ( order .eq. 5810 ) then
        call ld5810 ( x, y, z, w )
      else
        write ( *, '(a)' ) ' '
...
c*********************************************************************72
c
cc LD0006 computes the 6 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c  Parameters:
c
c    Output, double precision X(N), Y(N), Z(N), W(N), the coordinates
c    and weights of the points.
c
      implicit none
...
c*********************************************************************72
c
cc LD0014 computes the 14 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0026 computes the 26 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0038 computes the 38 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0050 computes the 50 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0074 computes the 74 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0086 computes the 86 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0110 computes the 110 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0146 computes the 146 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0170 computes the 170 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0194 computes the 194 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0230 computes the 230 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0266 computes the 266 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0302 computes the 302 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0350 computes the 350 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0434 computes the 434 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0590 computes the 590 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0770 computes the 770 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD0974 computes the 974 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD1202 computes the 1202 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD1454 computes the 1454 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD1730 computes the 1730 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD2030 computes the 2030 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD2354 computes the 2354 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD2702 computes the 2702 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD3074 computes the 3074 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD3470 computes the 3470 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD3890 computes the 3890 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD4334 computes the 4334 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD4802 computes the 4802 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD5294 computes the 5294 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc LD5810 computes the 5810 point Lebedev angular grid.
c
c  Modified:
c
c    09 September 2010
...
c*********************************************************************72
c
cc ORDER_TABLE returns the order of a Lebedev rule.
c
c  Modified:
c
c    11 September 2010
...
c  Parameters:
c
c    Input, integer RULE, the index of the rule, between 1 and 65.
c
c    Output, integer ORDER_TABLE, the order of the rule.
c
      implicit none
...
c*********************************************************************72
c
cc PRECISION_TABLE returns the precision of a Lebedev rule.
c
c  Modified:
c
c    11 September 2010
...
c    Input, integer RULE, the index of the rule, between 1 and 65.
c
c    Output, integer PRECISION_TABLE, the precision of the rule.
c
      implicit none
...
c*********************************************************************72
c
cc XYZ_TO_TP converts (X,Y,Z) to (Theta,Phi) coordinates on the unit sphere.
c
c  Modified:
c
c    09 September 2010
...
c  Parameters:
c
c    Input, double precision X, Y, Z, the Cartesian coordinates of a point
c    on the unit sphere.
c
c    Output, double precision T, P, the Theta and Phi coordinates of
c    the point.
c
      implicit none
      ...
      if ( 0.0D+00 .lt. fact ) then
        t = acos ( x / fact )
      else
        t = acos ( x )
      end if

      if ( y .lt. 0.0D+00 ) then
        t = - t
      end if
c
