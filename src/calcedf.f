...
c
      ...
      do i=1,ndets
         ...
         enddo
         if (mal.ne.mbe) then
           stop 'calcedf.f: Alpha and Beta electrons are not equal'
         endif
c
c........Alpha electrons
c
...
c
     ...
     &     access='direct',recl=RecLength*2*nproba,form='unformatted')
      ...
      do i=1,np
        ...
        if (mulliken) jend=np
        do j=1,jend
          read (indconf,rec=j) (ordja(m+ncore),m=1,mal-ncore)
          do m=1,mal-ncore
...
c
          ...
          irec=i*(i-1)/2+j
          if (mulliken) irec=(i-1)*np+j
          write (indprob,rec=irec)(proba(n),n=1,nproba)
        enddo
      enddo
...
c
     ...
     &       '# Probability_ALPHA x Probability_BETA :', iper,' % done'
           ...
           if (mulliken) then
             ireca=(kwa(i)-1)*np+kwa(j)
             irecb=(kwb(i)-1)*np+kwb(j)
           else
             ireca=imaxa*(imaxa-1)/2+imina
             irecb=imaxb*(imaxb-1)/2+iminb
           endif
           read (indprob,rec=ireca)(proba(k),k=1,nproba)
           read (indprob,rec=irecb)(probb(k),k=1,nprobb)
c
