...
c-----------------------------------------------------------------------
c
      subroutine udrvother (aominp,mxcent,covx,epsneg,maxb,nloctot,
...
c
     ...
     &    nloctarget,nab,udat,epsneg,mal,mbe,largwr,warn,lw,lerr)
        ...
        if (nloc.ne.nloctarget) then
          stop ' udrvother.f: Failed localization of Canonical MOs'
        endif
c
c-----------------------------------------------------------------------
c
...
c
      ...
      allocate (ocup(1:nloctot),stat=ier)
      if (ier.ne.0) stop '# udrvother.f: Cannot allocate ocup()'
...
c
      ...
      if (ier.ne.0) stop '# udrvother.f: Cannot deallocate ocup()'
c
...
c
 ...
 100  format (' # udrvother.f: File ',a,' NOT FOUND')
