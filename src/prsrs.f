...
c-----------------------------------------------------------------------
c
      subroutine prsrs (epsdet,ndets,okord,ngroup,nta,ntb,nmo,ncore,
     &                  nel,lw,lr,maxp,minp,probal,line)
      ...
      character*(*) line
      logical setint,ok,okord,docalc
c
      okord=.true.
c
      probal=zero
...
c
 ...
 123    format (3(' #',/),
     ...
     &   3(' #',/))
      ...
      enddo
*     if (ndets.eq.1) then
*       call sdwsumdet (epsdet,prob,nttim(1),nttim(2),nta,ntb,wca,
*    &    wcb,ngroup,nmo)
*     else
